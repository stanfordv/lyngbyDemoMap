...
FrbKort = {
    ...
    setTitle: function() {
        ...
        // console.log("title " + this.config.text.title)
        // console.log("here " +JSON.stringify($('.panel-group#textcontainer>h4').html(this.config.text.title)))
        ...
        if (typeof config.data == 'string') {
            $.ajax({
                ...
                success: this.bind(function(config, data) {
                    ...
                    console.log("vector2 "+config.vector[0])
...
console.log("vector3")
    ...
    setColor: function(config) {
        ...
        for (var i = 0; i < config.data.length; i++) {
            ...
            if (!config.dates[config.data[i].navn]) {
                console.log("no name!!")
                config.dates[config.data[i].navn] = [];
            }
            ...
            config.dates[config.data[i].navn].push(obj)
        }
        for (var name in config.dates) {
            config.dates[name].sort(function(a, b) {
                return a.val - b.val });
    ...
    getRowFromData: function(type_id, distrikt_id) {
        var cd = this.current.dates[this.current.date];
              // console.log("typeof cd "+typeof cd)
        if (typeof cd !== 'undefined') {
            // console.log("inside")
            // console.log("incomeing "+this.current.dates[this.current.date])
    ...
    showHistory: function(config) {
        ...
        for (var name in hist) {
            ...
            // console.log("avg "+hist[name].avg.toFixed(config.decimals ||  0))
    ...
    showSlider: function(config) {
        ...
        input.slider({
            ...
            min: 0,
            max: dates.length - 1,
            value: 1, //jQuery.inArray(config.startDate,dates),
...
console.log("vector4")
    ...
    getHistory: function() {
        ...
        for (var name in this.current.dates) {
            ...
            var max = -999;
            var min = 999;
            // console.log("this.current.dates[name].length "+ this.current.dates[name].length)

            for (var i = 0; i < this.current.dates[name].length; i++) {
                // console.log("distrikt_id " + this.current.dates[name][i].distrikt_id)
                ...
                if (this.current.dates[name][i].type_id == this.currentVector) {
                    ...
                    // console.log("this.current.dates[name][i].val "+ this.current.dates[name][i].val)
            ...
            //todo: this avg is not working out - its undefined causing and error. 
            //more specifically val comes in as a value in fre version and always 0 in ltk ver.
            dates[name].avg = val / count;

            // console.log("avg "+ val/count)
    ...
    showVector: function(name, date) {
        ...
        // console.log("this.current.date" + this.current.date)
        // console.log("this.currentVector " + this.currentVector);
        ...
        if (!this.vector[this.currentVector]) {
            ...
            $.ajax({
                ...
                success: this.bind(function(vector, data) {
                     ...
                     // console.log("vector " + vector.data);
                    vector.data = data;
    ...
    setMap: function() {
        ...
        var LTKOuterBorder = L.geoJson(ltkOuterBorder).addTo(this.map);
        var highlightStyle = {
            "color": "#000000",
            "weight": 1

        };
        LTKOuterBorder.setStyle(highlightStyle);
